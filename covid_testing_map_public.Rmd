...
all_existing_sites <- rbind(health_centers %>% select(Name = `Health Center Name`, 
                                                      address = `Health Center address`), 
                            test_sites_hc %>% select(Name = `System or Hospital`, address))

all_existing_sites[c(23, 34, 35),1] <- c('Flu Clinic at Mercy Health Bond Hill',
                                         'The Jewish Hospital Mercy Health Outpatient Clinic',
                                         'Mercy Health Forest Hills Family Medicine')

all_existing_sites_10min_walk <- st_union(health_center_isochrones_10min_walk, test_sites_isochrones_10min_walk)
all_existing_sites_15min_drive <- st_union(health_center_isochrones_15min_drive, test_sites_isochrones_15min_drive)

d_neigh <- d_neigh %>%
  mutate(neigh_area = as.numeric(st_area(.)),
         neigh_intersection = map(row_number(), ~st_intersection(d_neigh[.x,], all_existing_sites_10min_walk)),
         intersection_area = as.numeric(map(neigh_intersection, st_area)),
         intersection_area = ifelse(is.na(intersection_area), 0, intersection_area),
         frac_coverage = intersection_area/neigh_area) %>%
  select(-neigh_area, -neigh_intersection, -intersection_area)
...
tm <- tm_shape(d_neigh, 
  ...
  tm_dots(size = 0.2, 
          col = "gray",
          alpha = 0.7) +
  tm_shape(all_existing_sites, name = 'Existing Testing Locations') +
  tm_dots(size = 0.2, 
          col = "white",
          alpha = 0.7) +
  tm_shape(all_existing_sites_10min_walk, 
           name = '10-min walk buffer to test site') +
  tm_polygons(col = 'white', alpha = 0.7) +
  tm_shape(all_existing_sites_15min_drive, 
